...
const OPENVPN_DIR = "/root/openvpn-ca"; // Path to OpenVPN setup
...
app.post("/api/open-vpn", async (req, res) => {
  try {
    ...
    switch (country) {
      case 'sfo':
        const sfodata = await axios.post(`http://164.92.123.161:3000/api/open-vpn`, req.body);
        return res.status(200).json(sfodata.data);
        break;
      case 'ams':
        const amsdata = await axios.post(`http://178.128.250.136:3000/api/open-vpn`, req.body);
        return res.status(200).json(amsdata.data);
        break;
      case 'ny':
    ...
    `], { cwd: OPENVPN_DIR }, (error, stdout, stderr) => {
        ...
        genReqProcess.on("close", (code) => {
          ...
          send "yes\r"
          expect "/root/openvpn-ca/pki/private/ca.key:"
          send "Admin@123\r"
          expect eof
...
${key}
      ...
      default:
        res.status(200).json({ success: false, message: "Invalid country", data: {} });
    }
...
app.post('/api/connect', async (req, res) => {
  ...
  switch (country) {
    case 'sfo':
      const sfodata = await axios.post(`http://164.92.123.161:3000/api/connect`, req.body);
      return res.status(200).json(sfodata.data);
      break;
    case 'ams':
      const amsdata = await axios.post(`http://178.128.250.136:3000/api/connect`, req.body);
      return res.status(200).json(amsdata.data);
      break;
    case 'ny':
      ...
      const keyPair = generateWireGuardKeyPair(udid);

      if (!keyPair) {
        return res.status(200).json({ success: false, config: "", user: {}, message: "Unable to generate Key pair" });
      }
      //const keyPair = generateKeyPair(udid);
      const publicKey = keyPair.publicKey;
      const privateKey = keyPair.privateKey;
      const ipAddress = getNextIPAddress(udid);
      ...
      break;
    default:
     return res.status(200).json({ success: false, message: "Invalid country" });
  }
...
//         if (!keyPair) {
//           res.status(200).json({ success: false, config: "", user: {}, message: "Unable to generate Key pair" });
//         }
//         //const keyPair = generateKeyPair(udid);
//         const publicKey = keyPair.publicKey;
//         const privateKey = keyPair.privateKey;
...
app.get("/api/fetch-servers", async (req, res) => {
  try {
    return res.status(200).json({
      success: true,
      data: serverList,
...
function generateConfigFile(publicKey, privateKey, ipAddress) {
  try {
    const configFile = `
    [Interface]
    PrivateKey = ${privateKey}
